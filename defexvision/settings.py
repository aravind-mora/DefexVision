...
# CSRF: trust the site's own domain(s) for POST forms in production
...
# Security hardening (recommended when serving over HTTPS)
if not DEBUG:
    SECURE_SSL_REDIRECT = _b("DJANGO_SECURE_SSL_REDIRECT", "False").lower() in ("1", "true")
    SESSION_COOKIE_SECURE = _b("DJANGO_SESSION_COOKIE_SECURE", "False").lower() in ("1", "true")
    CSRF_COOKIE_SECURE = _b("DJANGO_CSRF_COOKIE_SECURE", "False").lower() in ("1", "true")
